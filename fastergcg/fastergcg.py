...
logger = logging.getLogger("fastergcg")
...
class FasterGCGConfig:
    ...
    # lambda
    lambda_reg_embeddings_distance: float = 0.01


@dataclass
class FasterGCGResult:
    best_loss: float
    best_string: str
    losses: List[float]
    strings: List[str]
...
def sample_ids_from_grad(
    ...
    # print(topk_ids.shape)  # 20 256

    sampled_ids_pos = torch.argsort(torch.rand((search_width, n_optim_tokens), device=grad.device))[..., :n_replace]
    # print(sampled_ids_pos.shape)  # 512 1
    ...
    # ).squeeze(2)
    # Greedy selection: Always select the top one (greedy approach)
    sampled_ids_val = torch.gather(
        topk_ids[sampled_ids_pos],  # We take from the topk ids
        2,  # This represents the token replacement at each position
        torch.zeros((search_width, n_replace), dtype=torch.long, device=grad.device).unsqueeze(-1)  # Select the first (best) token
    ).squeeze(2)
...
class FasterGCG:
    def __init__(
        ...
        # fasterGCG
        self.lambda_reg_embeddings_distance = config.lambda_reg_embeddings_distance
    ...
    def run(
        ...
        for _ in tqdm(range(config.num_steps)):
            ...
            embeddings_j = self.model.get_input_embeddings().weight.data  # [vocab_size, embed_dim]
            embeddings_i = embeddings_j[buffer.get_best_ids().squeeze(0)]  # [adversarial_tokens_length, embed_dim]
            # print(embeddings_j.shape, embeddings_i.shape)
            embeddings_distance = torch.norm(embeddings_i[:, None] - embeddings_j[None, :], dim=-1)
            optim_ids_onehot_grad += self.lambda_reg_embeddings_distance * embeddings_distance
            with torch.no_grad():
                ...
                # print(sampled_ids[0])  # 512 20
                # while True: pass
                # Avoid the self loop
                # print(sampled_ids.shape)
                sampled_ids = filter_sampled_ids(sampled_ids, history_set)
                # print(sampled_ids.shape)

                if sampled_ids.numel() == 0:
                    # 重新生成sampled_ids，或者采取其他处理方式
                    pass
...
def tensor_to_hash(tensor: torch.Tensor) -> str:
    """Converts a tensor to a hashable string representation.
    
    Args:
        tensor: The tensor to hash.
    
    Returns:
        A hash string representing the tensor.
    """
    tensor_bytes = tensor.cpu().numpy().tobytes()  # Convert tensor to bytes for hashing
    return hashlib.sha256(tensor_bytes).hexdigest()  # Use SHA-256 to get a unique hash

def filter_sampled_ids(sampled_ids: Tensor, history_hash_set: set) -> Tensor:
    """Filters out the sampled_ids that have been previously seen (based on hashed values).
    
    Args:
        sampled_ids : Tensor, shape = (search_width, n_optim_ids)
            The newly sampled token ids.
        history_hash_set : set
            A set containing previously hashed sampled_ids for fast lookup.
    
    Returns:
        filtered_ids : Tensor
            The sampled ids that are not in the history_hash_set.
    """
    # Convert sampled_ids to a hashable format
    sampled_ids_hash = tensor_to_hash(sampled_ids)

    # Check if this hash already exists in the history set
    if sampled_ids_hash in history_hash_set:
        # If the hash is already in the set, return an empty tensor (indicating it is a duplicate)
        return torch.empty_like(sampled_ids)  # Or you could resample here if necessary

    # Add the new hash to the history set
    history_hash_set.add(sampled_ids_hash)
    
    # Return the sampled_ids if they are unique
    return sampled_ids
...
def run(
    ...
    messages: Union[str, List[dict]],
    target: str,
    config: Optional[FasterGCGConfig] = None,
) -> FasterGCGResult:
    ...
    Returns:
        A FasterGCGResult object that contains losses and the optimized strings.
    """
    if config is None:
        config = FasterGCGConfig()
    ...
    gcg = FasterGCG(model, tokenizer, config)
